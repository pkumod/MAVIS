...
double query_plan_generator1::ordering_time_;
double query_plan_generator1::traversal_time_;
double query_plan_generator1::nd_time_;

unsigned Catalog1::get_edge_relation_cardinality(unsigned u, unsigned v) {
  return 0;
}

void query_plan_generator1::generate_query_plan_for_test(
    Graph *query_graph, std::vector<uint32_t> &order) {
  ...
  for (uint32_t u = 1; u < query_vertices_num; ++u) {
    if (query_graph->getVertexDegree(u) >
            query_graph->getVertexDegree(order[0]) &&
        (core_vertices_num == 0 || query_graph->getCoreValue(u))) {
      order[0] = u;
    }
  }
  ...
  for (uint32_t i = 1; i < query_vertices_num; ++i) {
    ...
    for (uint32_t u = 0; u < query_vertices_num; ++u) {
      if (!visited[u] &&
          (query_graph->getCoreValue(u) > 1 || i >= core_vertices_num)) {
    ...
    if (tie_vertices.size() != 1) {
      ...
      for (auto u : temp) {
        // Compute the number of vertices in the matching order that has at
        // least one vertex not in the matching order && connected with u.
        ...
        for (uint32_t j = 0; j < i; ++j) {
          ...
          ComputeSetIntersection::ComputeCandidates(uun, uun_count, u_fn.data(),
                                                    (uint32_t)u_fn.size(),
    ...
    if (tie_vertices.size() != 1) {
      ...
      std::vector<uint32_t> u_fn;
      for (auto u : temp) {
        // Compute the number of vertices not in the matching order && not the
        // neighbor of vertices in the matching order, but is connected with u.
...
void query_plan_generator1::generate_query_plan_with_nd(
    Graph *query_graph, Catalog1 *storage,
  ...
  traversal_density_tree(query_graph, storage, density_tree, vertex_orders,
                         node_orders);
...
void query_plan_generator1::construct_density_tree(
    std::vector<nd_tree_node> &density_tree,
    std::vector<nd_tree_node> &k12_tree, std::vector<nd_tree_node> &k23_tree,
    std::vector<nd_tree_node> &k34_tree) {
  /** Eliminate the duplicate nodes.
   *     1. For each nucleus in k12_tree, if it is a subset of the nucleus in
   * k23, then remove the subtree rooted at it from k12_tree.
   *     2. For each nucleus in k23_tree, if it is a subset of the nucleus in
   * k34, then remove the subtree rooted at it from k23_tree.
   */
  ...
  /**
   * Construct the tree.
   *     1. For the root node of each k23_tree, find the smallest (by
   * cardinality) 12 nucleus contain it and set the nucleus as its parent.
   *     2. For the root node of each k34_tree, find the smallest (by
   * cardinality) 12 or 23 nucleus contain it and set the nucleus as its parent.
   */
  merge_tree(density_tree);
}

void query_plan_generator1::eliminate_node(
    std::vector<nd_tree_node> &density_tree,
      ...
      while (!q.empty()) {
        ...
        for (auto &target_node_id : roots) {
          ...
          if (cur_node.vertices_.size() <= target_node.vertices_.size()) {
            ...
            ComputeSetIntersection::ComputeCandidates(
                (uint32_t *)cur_node.vertices_.data(),
        ...
        if (!remove) {
          ...
          if (density_tree.back().parent_ != -1)
            density_tree[density_tree.back().parent_].children_.push_back(
                new_id);
...
void query_plan_generator1::merge_tree(
    std::vector<nd_tree_node> &density_tree) {
  for (int i = density_tree.size() - 1; i >= 0; --i) {
    ...
    if (cur_node.parent_ == -1) {
      for (int j = i - 1; j >= 0; --j) {
        ...
        if (target_node.r_ < cur_node.r_ &&
            target_node.vertices_.size() < smallest_cardinality) {
          uint32_t cn_cnt = 0;
          ComputeSetIntersection::ComputeCandidates(
              (uint32_t *)cur_node.vertices_.data(),
              (uint32_t)cur_node.vertices_.size(),
...
void query_plan_generator1::print_density_tree(
    std::vector<nd_tree_node> &density_tree) {
  printf("Density Tree:\n");
  std::queue<int> q;
  q.push(0);
  ...
  while (!q.empty()) {
    ...
    q.pop();
    printf(
        "\tid %d, parent %d, k %d, r %d, s %d, |V| %zu, |E| %d, density %.4f, "
        "num of children %zu: ",
        density_tree[node_id].id_, density_tree[node_id].parent_,
        density_tree[node_id].k_, density_tree[node_id].r_,
        density_tree[node_id].s_, density_tree[node_id].vertices_.size(),
        density_tree[node_id].num_edges_, density_tree[node_id].density_,
        density_tree[node_id].children_.size());
...
void query_plan_generator1::traversal_density_tree(
    Graph *query_graph, Catalog1 *storage,
    std::vector<nd_tree_node> &density_tree,
    std::vector<std::vector<uint32_t>> &vertex_orders,
    std::vector<std::vector<uint32_t>> &node_orders) {
  ...
  for (auto &node : density_tree) {
    if (node.children_.empty()) {
      traversal_node(query_graph, storage, density_tree, vertex_order,
                     node_order, visited_vertex, visited_node, node,
                     extendable_vertex);
...
void query_plan_generator1::traversal_node(
    Graph *query_graph, Catalog1 *storage,
    std::vector<nd_tree_node> &density_tree,
  ...
  if (all_vertices_visited) {
    if (cur_node.parent_ != -1 && !visited_node[cur_node.parent_])
      traversal_node(query_graph, storage, density_tree, vertex_order,
                     node_order, visited_vertex, visited_node,
  ...
  while (flag) {
    ...
    for (auto child_id : cur_node.children_) {
      if (!visited_node[child_id]) {
        double connectivity = connectivity_common_neighbors(
            visited_vertex, density_tree[child_id]);
        if (connectivity > max_connectivity ||
            (connectivity == max_connectivity &&
             density_tree[child_id].density_ >
                 density_tree[selected_child].density_) ||
            (connectivity == max_connectivity &&
             density_tree[child_id].density_ >
                 density_tree[selected_child].density_ &&
             density_tree[child_id].vertices_.size() >
                 density_tree[selected_child].vertices_.size())) {
    ...
    if (flag) {
      if (max_connectivity < 2) {
        connectivity_shortest_path(query_graph, storage, vertex_order,
                                   visited_vertex, cur_node, prev, dist);
        max_connectivity = 0;
        ...
        for (auto rb = vertex_in_shortest_path.rbegin();
             rb != vertex_in_shortest_path.rend(); rb++) {
          if (!visited_vertex[*rb]) {
            vertex_order.push_back(*rb);
            visited_vertex[*rb] = true;
            update_extendable_vertex(query_graph, *rb, extendable_vertex,
                                     visited_vertex);

            {
              ...
              greedy_expand(query_graph, storage, vertex_order, visited_vertex,
                            extendable_vertex, bn_cnt_threshold);
            }
          }
        }
      }
  ...
  if (vertex_order.empty()) {
        ...
        if (u < v && query_graph->checkEdgeExistence(u, v)) {
          if (local_degree_sum > max_degree_sum ||
              (local_degree_sum == max_degree_sum &&
               storage->get_edge_relation_cardinality(u, v) <
                   min_relation_size)) {
    ...
    update_extendable_vertex(query_graph, first_vertex, extendable_vertex,
                             visited_vertex);

    vertex_order.push_back(second_vertex);
    visited_vertex[second_vertex] = true;
    update_extendable_vertex(query_graph, second_vertex, extendable_vertex,
                             visited_vertex);
  }
  ...
  while (!all_vertices_visited) {
    ...
    for (auto u : cur_node.vertices_) {
      if (!visited_vertex[u]) {
        ...
        if (local_bn_cnt > max_bn ||
            (local_bn_cnt == max_bn &&
             local_candidate_vertex_num < min_candidate_vertex_num) ||
            (local_bn_cnt == max_bn &&
             local_candidate_vertex_num == min_candidate_vertex_num &&
             u_deg > max_degree)) {
          selected_vertex = u;
    ...
    if (!all_vertices_visited) {
      ...
      update_extendable_vertex(query_graph, selected_vertex, extendable_vertex,
                               visited_vertex);
    }
  }
  {
    // uint32_t bn_cnt_threshold = 2;
    // if (cur_node.parent_ != -1 && density_tree[cur_node.parent_].r_ >
    // bn_cnt_threshold) {
  ...
  if (cur_node.parent_ != -1 && !visited_node[cur_node.parent_])
    traversal_node(query_graph, storage, density_tree, vertex_order, node_order,
                   visited_vertex, visited_node, density_tree[cur_node.parent_],
                   extendable_vertex);
}

double query_plan_generator1::connectivity_common_neighbors(
    std::vector<bool> &visited_vertex, nd_tree_node &child_node) {
...
void query_plan_generator1::connectivity_shortest_path(
    Graph *query_graph, Catalog1 *storage, std::vector<uint32_t> &vertex_order,
  ...
  priority_queue<std::pair<double, uint32_t>,
  ...
  while (!min_pq.empty()) {
    ...
    for (uint32_t i = 0; i < nbrs_cnt; ++i) {
      ...
      if (in_cur_node[v] && !visited_vertex[v]) {
        double distance =
            storage->num_candidates_[v] > 3 ? storage->num_candidates_[v] : 3;
...
void query_plan_generator1::print_vertex_orders(
    Graph *query_graph, std::vector<std::vector<uint32_t>> &vertex_orders,
    std::vector<std::vector<uint32_t>> &node_orders) {
  ...
  for (uint32_t i = 0; i < vertex_orders.size(); ++i) {
    ...
    uint32_t utility_value =
        query_plan_utility_value(query_graph, vertex_orders[i], bn_cnt_list);
...
void query_plan_generator1::query_plan_correctness_check(
    Graph *query_graph, std::vector<uint32_t> &vertex_order) {
  uint32_t n = query_graph->getVerticesCount();
  std::vector<bool> visited(n, false);
  if (vertex_order.size() != n) {
    printf(
        "The number of vertices in the query plan is not correct %zu : %d.\n",
        vertex_order.size(), n);
    exit(-1);
  }
  for (auto u : vertex_order) {
...
uint32_t query_plan_generator1::query_plan_utility_value(
    Graph *query_graph, std::vector<uint32_t> &vertex_order,
...
void query_plan_generator1::update_extendable_vertex(
    Graph *query_graph, uint32_t u,
    std::unordered_set<uint32_t> &extendable_vertex,
...
void query_plan_generator1::greedy_expand(
    Graph *query_graph, Catalog1 *storage, std::vector<uint32_t> &vertex_order,
...
void query_plan_generator1::print_metrics() {
