...
void RMFilter::filter_from_view(catalog* storage) {
    auto start = std::chrono::high_resolution_clock::now();
    std::vector<int> attach_edge(vertices_count_, -1);
    std::set<std::pair<uint32_t, uint32_t>> visited_edges;
    for (uint32_t u = 0; u < vertices_count_; ++u) {
        ...
        for (uint32_t i = 0; i < u_nbrs_cnt; ++i) {
            ...
            if (!rq_.filtered_edge_lists_[u][v].empty()) {
                attach_edge[u] = v;
                attach_edge[v] = u;
                visited_edges.insert(std::make_pair(u, v));
            }
        }
    }
    ...
    for (;;) {
        ...
        for (uint32_t i = 0; i < vertices_count_; ++i) {
            if (attach_edge[i] == -1) {
                for (uint32_t j = 0; j < vertices_count_; ++j) {
                    if (query_graph_->checkEdgeExistence(i, j) && attach_edge[j] != -1) {
                        filter(j, i, attach_edge[j], storage);
                        attach_edge[i] = j;
                        if (i < j) {
                            visited_edges.insert(std::make_pair(i, j));
                        } else {
                            visited_edges.insert(std::make_pair(j, i));
                        }
                        changed = true;
                    }
                }
    ...
    for (uint32_t u = 0; u < vertices_count_; ++u) {
        ...
        for (uint32_t i = 0; i < u_nbrs_cnt; ++i) {
            ...
            if (visited_edges.find(std::make_pair(u, v)) == visited_edges.end()) {
                filter(u, v, attach_edge[u], storage);
            }
        }
    }
    auto end = std::chrono::high_resolution_clock::now();
    semi_join_time_ += std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
}

void RMFilter::execute(const Graph* query_graph, const Graph* data_graph, catalog* storage, bool enable_elimination) {
    std::cout << "enable_elimination: " << enable_elimination << std::endl;
     ...
     * NLF filter.
     */
    filter->execute(storage, rq_.need_nlf_);
    auto end = std::chrono::high_resolution_clock::now();
    filter_time_ = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    std::cout << "nlf filter time(ms): " << filter_time_ / 1000000. << std::endl;
#endif
    ...
    if (enable_elimination) {
        semi_join_time_ = 0;
        semi_join_operator_ = std::make_unique<semi_join>(data_graph_->getVerticesCount());
        bool used_view = false;
        for (ui u = 0; u < vertices_count_; ++u) {
            for (ui u1 = u + 1; u1 < vertices_count_; ++u1) {
                if (query_graph_->checkEdgeExistence(u, u1) && !rq_.filtered_edge_lists_[u][u1].empty()) {
                    used_view = true;
                    break;
                }
            }
        ...
        if (used_view)
            filter_from_view(storage);
        else
            top_down_filter(storage);
        bottom_up_filter(storage);
    }
...
void RMFilter::initialize(const Graph* query_graph, const Graph* data_graph) {
...
void RMFilter::scan_relation(catalog* storage) {
    ...
    for (uint32_t u = 0; u < vertices_count_; ++u) {
        ...
        for (uint32_t i = 0; i < u_nbrs_cnt; ++i) {
            ...
            if (u > v) continue;
            if (!rq_.filtered_edge_lists_[u][v].empty()) {
                storage->edge_relations_[u][v].size_ = rq_.filtered_edge_lists_[u][v].size();
                storage->edge_relations_[u][v].edges_ = new Edge[rq_.filtered_edge_lists_[u][v].size()];
                memcpy(storage->edge_relations_[u][v].edges_, rq_.filtered_edge_lists_[u][v].data(), sizeof(Edge) * rq_.filtered_edge_lists_[u][v].size());
            } else {
                uint32_t u_label = query_graph_->getVertexLabel(u);
                uint32_t v_label = query_graph_->getVertexLabel(v);
                scan_operator->execute(u_label, v_label, &(storage->edge_relations_[u][v]));
            }
        }
    }
...
void RMFilter::filter(uint32_t mid, uint32_t l, uint32_t r, catalog* storage) {
    uint32_t lkp;
    edge_relation* l_relation = get_key_position_in_relation(mid, l, storage, lkp);
    uint32_t rkp;
    edge_relation* r_relation = get_key_position_in_relation(mid, r, storage, rkp);
    storage->num_candidates_[mid] = semi_join_operator_->execute(l_relation, lkp, r_relation, rkp);
}

void RMFilter::bottom_up_filter(catalog* storage) {
    auto start = std::chrono::high_resolution_clock::now();
    auto end = std::chrono::high_resolution_clock::now();
    // Bottom-up semi-join along the degeneracy ordering for the non-core
    // vertices.
    ...
    // Perform the full reducer for the star rooted at each core vertex along the
    // degeneracy ordering.
    start = std::chrono::high_resolution_clock::now();
    for (uint32_t i = non_core_vertices_count_; i < vertices_count_; ++i) {
        ...
        for (int j = 0; j < static_cast<int>(u_nbrs_cnt) - 1; ++j) {
            filter(u, u_nbrs[j + 1], u_nbrs[j], storage);
        }
    ...
    semi_join_time_ += bottom_up_non_core_semi_join_time_ + bottom_up_core_semi_join_time_ ;
}

void RMFilter::top_down_filter(catalog* storage) {
    auto start = std::chrono::high_resolution_clock::now();
    auto end = std::chrono::high_resolution_clock::now();
    // Perform the full reducer for the star rooted at each core vertex along the
    // reverse of the degeneracy ordering.
    start = std::chrono::high_resolution_clock::now();
    ...
    // Top-down semi-join along the reverse of degeneracy ordering for the
    // non-core vertices.
    start = std::chrono::high_resolution_clock::now();
    for (int i = static_cast<int>(non_core_vertices_count_) - 1; i >= 0; --i) {
        // If all vertices are non-core vertices, then skip the last vertex in the
        // degeneracy ordering.
        if (i == vertices_count_ - 1) continue;
    ...
    semi_join_time_ += top_down_non_core_semi_join_time_ + top_down_core_semi_join_time_;
}

void RMFilter::eliminate_dangling_tuples(catalog* storage) {


}

void RMFilter::generate_preprocess_plan() {
    GraphOperations::compute_degeneracy_order(query_graph_, degeneracy_ordering_);
    for (uint32_t i = 0; i < vertices_count_; ++i) {
        vertices_index_[degeneracy_ordering_[i]] = i;
    }

    if (non_core_vertices_count_ == 0) return;
...
edge_relation* RMFilter::get_key_position_in_relation(uint32_t u,
                                                          uint32_t v,
                                                          catalog* storage,
                                                          uint32_t& kp) {
    kp = 0;
