...
private:
    ...
    std::unique_ptr<semi_join> semi_join_operator_;
...
private:
    ...
    void bottom_up_filter(catalog* storage);
    void top_down_filter(catalog* storage);

    void generate_preprocess_plan();

    void filter_from_view(catalog* storage);

    edge_relation* get_key_position_in_relation(uint32_t u, uint32_t v, catalog *storage, uint32_t &kp);
    void filter(uint32_t mid, uint32_t l, uint32_t r, catalog* storage);

public:
    RMFilter(RewriteQuery& rq) : rq_(rq), vertices_count_(0), non_core_vertices_count_(0), degeneracy_ordering_(nullptr), vertices_index_(nullptr),
