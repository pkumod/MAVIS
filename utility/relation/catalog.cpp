...
void catalog::load_precomputed_embeddings(std::vector<std::vector<unsigned>>& embeddings) {
    precomputed_relations_ = new flat_relation(embeddings[0].size());
    for (auto embedding: embeddings) {
        precomputed_relations_->push_back(embedding.data());
    }
}

void catalog::build_reverse_index(int u) {
    reverse_indices_[u] = new CompressReverseIndex(max_data_vertex_id_, candidate_sets_[u], num_candidates_[u]);
}