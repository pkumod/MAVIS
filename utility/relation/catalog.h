...
class catalog {
   ...
   public:
    ...
    CompressReverseIndex** reverse_indices_;
    ...
    flat_relation* precomputed_relations_;
   ...
   public:
    explicit catalog(Graph* query_graph, Graph* data_graph) {
        ...
        memset(num_candidates_, 0, sizeof(uint32_t) * num_sets_);
        ...
        precomputed_relations_ = nullptr;
        reverse_indices_ = new CompressReverseIndex*[num_sets_];

        for (uint32_t i = 0; i < num_sets_; ++i) {
            candidate_sets_[i] = nullptr;
            reverse_indices_[i] = nullptr;
    ...
    ~catalog() {
        for (uint32_t i = 0; i < num_sets_; ++i) {
            ...
            if (reverse_indices_[i] != nullptr) {
                delete reverse_indices_[i];
            }
    ...
    void load_precomputed_embeddings(std::vector<std::vector<unsigned>>& embeddings);

    void build_reverse_index(int u);

    bool reverse_index_built(int u) {
        return reverse_indices_[u] != nullptr;
    }
};
