...
uint64_t leapfrogtriejoin::execute() {
  ...
  // std::cout << "core_vertex_depth: " << core_vertex_depth << std::endl;
  // std::cout << "start_depth: " << start_depth << std::endl;
  // std::cout << "input_: " << std::endl;
  // for (unsigned i = 0; i < input_->get_size(); ++i) {
  //   for (unsigned j = 0; j < input_->get_tuple_length(); ++j) {
  //     std::cout << input_->get_tuple(i)[j] << " ";
  //   }
  //   std::cout << std::endl;
  // }
  ...
  for (uint64_t i = 0; i < input_->get_size(); ++i) {
    ...
    if (start_depth >= num_core_vertex_) {
      ...
      local_candidates_[start_depth] = catalog_->get_non_core_relation_children(
          temp_bn, temp_u, temp_key, num_local_candidates_[start_depth]);
      ...
      if (num_local_candidates_[start_depth] == 0) {
        vec_failing_set_[start_depth - 1] =
            ancestors_[vertex_ordering_[start_depth]];
      } else {
      ...
      if (num_local_candidates_[cur_depth] == 0) {
        vec_failing_set_[cur_depth - 1] =
            ancestors_[vertex_ordering_[cur_depth]];
      } else {
      ...
      while (true) {
        while (idx_[cur_depth] < num_local_candidates_[cur_depth]) {
          ...
          num_local_candidates_[cur_depth + 1] =
              compute_local_candidates(cur_depth + 1);

#ifdef FAILING_SET_PRUNING
          if (num_local_candidates_[cur_depth + 1] == 0) {
            vec_failing_set_[cur_depth] =
                ancestors_[vertex_ordering_[cur_depth + 1]];
          } else {
            vec_failing_set_[cur_depth].reset();
          }
#endif

          intermediate_result_count_[cur_depth + 1] +=
              num_local_candidates_[cur_depth + 1];

          if (cur_depth == max_depth - 2) {
#if defined(HOMOMORPHISM) && defined(OUTPUT_OPTIMIZATION) && \
    RELATION_STRUCTURE == 0
            ...
            for (uint32_t x = 0; x < loop_count; ++x) {
              ...
              if (count_ < OUTPUT_RESULT_NUM_LIMIT) {
                ...
                memcpy(current_position_, embedding_,
                       num_vertex_ * sizeof(uint32_t));
            ...
            for (uint32_t j = 0; j < loop_count; ++j) {
              ...
              if (visited_[temp_v]) {
                ...
                vec_failing_set_[cur_depth + 1] |=
                    ancestors_[reverse_embedding_[temp_v]];
...
#endif
#ifdef FAILING_SET_PRUNING
            if (!vec_failing_set_[cur_depth].test(
                    vertex_ordering_[cur_depth])) {
              vec_failing_set_[cur_depth - 1] = vec_failing_set_[cur_depth];
            ...
            for (uint32_t j = 0; j < num_local_candidates_[next_depth]; ++j) {
              ...
              // local_candidates_[next_depth][j] =
              // catalog_->candidate_sets_[next_vertex][temp_idx];
...
bool leapfrogtriejoin::enumerate_non_core_results(uint32_t start_depth,
                                                  uint32_t max_depth) {
  for (idx_[start_depth] = 0;
    ...
    local_candidates_[cur_depth] = catalog_->get_non_core_relation_children(
        bn, u, key, num_local_candidates_[cur_depth]);
    intermediate_result_count_[cur_depth] += num_local_candidates_[cur_depth];
#ifdef FAILING_SET_PRUNING
    if (num_local_candidates_[cur_depth] == 0) {
    ...
    if (cur_depth == max_depth - 1) {
      ...
      for (uint32_t j = 0; j < temp_count; ++j) {
        ...
        if (visited_[temp_buffer[j]]) {
          ...
          vec_failing_set_[cur_depth] |=
              ancestors_[reverse_embedding_[temp_buffer[j]]];
          vec_failing_set_[cur_depth - 1] |= vec_failing_set_[cur_depth];
#endif
          continue;
        }
...
#ifdef FAILING_SET_PRUNING
      if (!vec_failing_set_[cur_depth - 1].test(
              vertex_ordering_[cur_depth - 1])) {
      ...
      while (true) {
        while (idx_[cur_depth] < num_local_candidates_[cur_depth]) {
          ...
          local_candidates_[next_depth] =
              catalog_->get_non_core_relation_children(
                  bn, u, key, num_local_candidates_[next_depth]);
          intermediate_result_count_[next_depth] +=
              num_local_candidates_[next_depth];
#ifdef FAILING_SET_PRUNING
          if (num_local_candidates_[next_depth] == 0) {
            vec_failing_set_[next_depth - 1] =
                ancestors_[vertex_ordering_[next_depth]];
          } else {
...
uint32_t leapfrogtriejoin::compute_local_candidates(uint32_t depth) {
  ...
  if (bn_count == 1) {
...
#endif
    local_candidates_[depth] =
        catalog_->get_core_relation_children(bn, u, key, lc_count);
  } else {
    ...
    uint32_t* lc1 =
        catalog_->get_core_relation_children(bn1, u, key1, lc_count1);
    uint32_t* lc2 =
        catalog_->get_core_relation_children(bn2, u, key2, lc_count2);
    ...
    ComputeSetIntersection::ComputeCandidates(
        lc1, lc_count1, lc2, lc_count2, local_candidates_[depth], lc_count);
    ...
    cached_bsr_sets_[depth].size_ = intersect_qfilter_bsr_hybrid(
        bsr_set1.base_, bsr_set1.states_, bsr_set1.size_, bsr_set2.base_,
        bsr_set2.states_, bsr_set2.size_, cached_bsr_sets_[depth].base_,
        cached_bsr_sets_[depth].states_);
#endif
    ...
    for (uint32_t i = 2; i < bn_count; ++i) {
      ...
      ComputeSetIntersection::ComputeCandidates(
          lc1, lc_count1, local_candidates_[depth], lc_count, si_buffer_,
          temp_lc_count);
...
#ifdef COLLECT_SI_STATISTICS
      set_intersection_cost_[depth] +=
          std::max(bsr_set.size_, cached_bsr_sets_[depth].size_);
      set_intersection_count_[depth] += 1;
#endif

      buffer_bsr_set1_.size_ = intersect_qfilter_bsr_hybrid(
          bsr_set.base_, bsr_set.states_, bsr_set.size_,
...
#ifdef SPARSE_BITMAP
    lc_count = (uint32_t)offline_bsr_trans_uint(
        cached_bsr_sets_[depth].base_, cached_bsr_sets_[depth].states_,
    ...
    if (intersection_cache_type_[depth] == INTERSECTION_CACHE_TYPE::none ||
        !is_intersection_cache_valid(depth)) {
      uint32_t cached_bn_count =
          cached_bns_offset_[depth + 1] - cached_bns_offset_[depth];
      uint32_t* cached_bns = cached_bns_ + cached_bns_offset_[depth];
      uint32_t* cached_bns_depth =
          cached_bns_depth_ + cached_bns_offset_[depth];
      ...
      uint32_t* lc1 =
          catalog_->get_core_relation_children(cached_bn1, u, key1, lc_count1);
      uint32_t* lc2 =
          catalog_->get_core_relation_children(cached_bn2, u, key2, lc_count2);

#ifdef COLLECT_SI_STATISTICS
      set_intersection_cost_[depth] += std::max(lc_count1, lc_count2);
      set_intersection_count_[depth] += 1;
#endif
      ComputeSetIntersection::ComputeCandidates(lc1, lc_count1, lc2, lc_count2,
                                                cached_candidates_[depth],
...
#elif RELATION_STRUCTURE == 1 || RELATION_STRUCTURE == 2
      BSRSet& bsr_set1 =
          catalog_->bsr_relations_[cached_bn1][u].hash_bsrs_[key1];
      BSRSet& bsr_set2 =
          catalog_->bsr_relations_[cached_bn2][u].hash_bsrs_[key2];
#endif
      ...
      for (uint32_t i = 2; i < cached_bn_count; ++i) {
        ...
        lc_count1 = 0;
        lc1 =
            catalog_->get_core_relation_children(cached_bn, u, key, lc_count1);
#ifdef COLLECT_SI_STATISTICS
        set_intersection_cost_[depth] +=
            std::max(lc_count1, cached_candidates_count);
        set_intersection_count_[depth] += 1;
#endif
        ComputeSetIntersection::ComputeCandidates(
            lc1, lc_count1, cached_candidates_[depth], cached_candidates_count,
...
#elif RELATION_STRUCTURE == 1 || RELATION_STRUCTURE == 2
        BSRSet& bsr_set =
            catalog_->bsr_relations_[cached_bn][u].hash_bsrs_[key];
#endif
    ...
    if (intersection_cache_type_[depth] != INTERSECTION_CACHE_TYPE::partial) {
      ...
      uint32_t no_cached_bn_count =
          no_cached_bns_offset_[depth + 1] - no_cached_bns_offset_[depth];
      uint32_t* no_cached_bns = no_cached_bns_ + no_cached_bns_offset_[depth];
      uint32_t* no_cached_bns_depth =
          no_cached_bns_depth_ + no_cached_bns_offset_[depth];
      ...
      uint32_t lc_count1 = 0;
      uint32_t* lc1 =
          catalog_->get_core_relation_children(no_cached_bn, u, key, lc_count1);

#ifdef COLLECT_SI_STATISTICS
      set_intersection_cost_[depth] +=
          std::max(lc_count1, cached_candidates_count_[depth]);
      set_intersection_count_[depth] += 1;
#endif
      ComputeSetIntersection::ComputeCandidates(
          lc1, lc_count1, cached_candidates_[depth],
          cached_candidates_count_[depth], local_candidates_[depth], lc_count);
...
#elif RELATION_STRUCTURE == 1 || RELATION_STRUCTURE == 2
      BSRSet& bsr_set1 =
          catalog_->bsr_relations_[no_cached_bn][u].hash_bsrs_[key];
#endif

#ifdef COLLECT_SI_STATISTICS
      set_intersection_cost_[depth] +=
          std::max(bsr_set1.size_, cached_bsr_sets_[depth].size_);
      set_intersection_count_[depth] += 1;
#endif
      buffer_bsr_set1_.size_ = intersect_qfilter_bsr_hybrid(
          bsr_set1.base_, bsr_set1.states_, bsr_set1.size_,
          cached_bsr_sets_[depth].base_, cached_bsr_sets_[depth].states_,
          cached_bsr_sets_[depth].size_, buffer_bsr_set1_.base_,
          buffer_bsr_set1_.states_);
#endif
...
#ifndef SPARSE_BITMAP
        lc_count1 = 0;
        lc1 = catalog_->get_core_relation_children(no_cached_bn, u, key,
                                                   lc_count1);
...
#ifdef COLLECT_SI_STATISTICS
        set_intersection_cost_[depth] +=
            std::max(bsr_set.size_, buffer_bsr_set1_.size_);
        set_intersection_count_[depth] += 1;
#endif
        buffer_bsr_set2_.size_ = intersect_qfilter_bsr_hybrid(
            bsr_set.base_, bsr_set.states_, bsr_set.size_,
            buffer_bsr_set1_.base_, buffer_bsr_set1_.states_,
            buffer_bsr_set1_.size_, buffer_bsr_set2_.base_,
            buffer_bsr_set2_.states_);
  ...
  if (depth == num_vertex_ - 1) {
...
#else
    {
      ...
      uint32_t* last_vertex_candidate_sets =
          catalog_->candidate_sets_[last_vertex];
      uint32_t* temp_buffer = local_candidates_[depth];
      uint32_t loop_count = lc_count;
      switch (loop_count & 7) {
        case 7:
          temp_buffer[loop_count - 1] =
              last_vertex_candidate_sets[temp_buffer[loop_count - 1]];
          loop_count -= 1;
        case 6:
          temp_buffer[loop_count - 1] =
              last_vertex_candidate_sets[temp_buffer[loop_count - 1]];
...
void leapfrogtriejoin::initialize() {
  ...
  last_level_candidates_ =
      new uint32_t[catalog_->max_num_candidates_per_vertex_];
  ...
  std::fill(intermediate_result_count_,
            intermediate_result_count_ + num_vertex_, 0);
  ...
  std::fill(mapping_to_cached_bns_,
            mapping_to_cached_bns_ + num_vertex_ * num_vertex_,
            catalog_->max_data_vertex_id_ + 1);
#endif
  ...
  for (uint32_t i = 0; i < num_vertex_; ++i) {
    cached_bsr_sets_[i].states_ =
        new int[catalog_->max_num_candidates_per_vertex_];
    cached_bsr_sets_[i].base_ =
        new int[catalog_->max_num_candidates_per_vertex_];
  }
...
void leapfrogtriejoin::initialize_intersection_cache() {
  ...
  for (uint32_t i = 0; i < num_vertex_; ++i) {
    ...
    if (bns_offset_[i + 1] - bns_offset_[i] >= 2) {
      ...
      // If only the depth of the first backward neighbor is lower than i - 1,
      // then intersection cache is none. Otherwise, it is partial or full.
      if (cached_bn_offset - cached_bns_offset_[i] == 1) {
        no_cached_bn_offset = no_cached_bns_offset_[i];
        cached_bns_depth_[cached_bn_offset] =
            no_cached_bns_depth_[no_cached_bn_offset];
  ...
  for (uint32_t i = 0; i < num_vertex_; ++i) {
    if (i >= input_->get_tuple_length() &&
        bns_offset_[i + 1] - bns_offset_[i] >= 2) {
      cached_candidates_[i] =
          new uint32_t[catalog_->max_num_candidates_per_vertex_];
    } else {
...
bool leapfrogtriejoin::is_intersection_cache_valid(uint32_t depth) {
  bool valid = true;
  for (uint32_t i = cached_bns_offset_[depth];
       i < cached_bns_offset_[depth + 1]; ++i) {
