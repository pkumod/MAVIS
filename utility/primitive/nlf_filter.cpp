...
bool nlf_filter::check(const std::vector<std::pair<uint32_t, uint32_t>>* u_nlf, const std::vector<std::pair<uint32_t, uint32_t>>* v_nlf) {
    for (auto element : *u_nlf) {
        auto iter = std::lower_bound(v_nlf->begin(), v_nlf->end(), std::make_pair(element.first, 0), cmp);
        if (iter == v_nlf->end() || iter->first != element.first || iter->second < element.second) {
            return false;
        }
    }
    return true;
...
void nlf_filter::execute(std::vector<std::vector<uint32_t>>& candidate_sets) {
    ...
    for (uint32_t u = 0; u < n; ++u) {
        ...
        candidate_set.reserve(data_vertex_num);
...
void nlf_filter::execute(catalog* storage, std::vector<bool>& need_nlf) {
    for (uint32_t u = 0; u < query_graph_->getVerticesCount(); ++u) {
        if (!need_nlf[u]) continue;
        uint32_t u_nbrs_cnt;
        const uint32_t* u_nbrs = query_graph_->getVertexNeighbors(u, u_nbrs_cnt);
        if (u_nbrs_cnt <= 1) continue;
        uint32_t uu = u_nbrs[u_nbrs_cnt - 1];
