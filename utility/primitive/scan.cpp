...
void scan::execute(uint32_t src_label, uint32_t dst_label, edge_relation* relation) {
    if (data_graph_->enable_edge_index_) {
...
void scan::execute_with_index(uint32_t src_label, uint32_t dst_label, edge_relation* relation) {
    ...
    if (data_graph_->getEdgeIndex2()->contains(key)) {
        auto& edges = data_graph_->getEdgeIndex2()->at(key);
